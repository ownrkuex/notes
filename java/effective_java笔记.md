...
* 对于不适合或不可以使用继承的场合：
  ...
  * 数组和泛型的两个区别：
    * 泛型信息只有在编译期间才有，运行时没有（泛型擦除）。因此类型检查只在编译时进行。与此相反的是，数组提供了运行时的类型安全，但是没有编译时的类型安全。
    * 数组能协变，泛型不能协变也不能逆变。
  ...
  * 数组类型是不能带泛型的（编译错误），但是有一点例外，可变参数是可以带泛型的，比如`Arrays.asList(T... a)`，这个方法内部的a就是一个T[]数组。为了确保类型安全，不应该向a中写入任何东西，不应该发布a，并在方法上注解@SafeVarargs来向编译器保证该方法是类型安全的。从@SafeVarargs的语义中可以看出这个注解只能用于final方法或static方法。
  
  * 大量线程共享`java.util.Random`对象时，会有性能问题，应考虑使用`ThreadLocalRandom.current()`。